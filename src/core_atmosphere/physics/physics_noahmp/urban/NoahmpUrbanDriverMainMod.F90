...
contains
  ...
  SUBROUTINE noahmp_urban( NoahmpIO,sf_urban_physics,         NSOIL,     IVGTYP,ITIMESTEP,  & ! IN : Model configuration 
              ...
              FVEGXY,Q2MVXY,Q2MBXY,T2MVXY,T2MBXY, Q2,T2,TH2,U10,V10 ,jmonth,jday,SR_URB2D, SB_URB2D, SG_URB2D, RR_URB2D, RB_URB2D, RG_URB2D, HR_URB2D, HB_URB2D, HG_URB2D, ELER_URB2D, ELEB_URB2D, ELEG_URB2D, G0R_URB2D, G0B_URB2D, G0G_URB2D &  !,PSIM,PSIH,GZ1OZ0,AKHS,AKMS !cye-add 2025-03-26
                 )
...
! input variables surface_driver --> lsm
     ...
     REAL, OPTIONAL,                                         INTENT(INOUT   ) :: DECLIN_URB
...
!===hydrological variable for single layer UCM===
     ...
     INTEGER, INTENT(IN) :: jmonth, jday !cye add 2025-06-16
     REAL    :: DRELR_URB
     REAL    :: DRELB_URB
     REAL    :: DRELG_URB
     REAL    :: FLXHUMR_URB
...
! output variable lsm --> surface_driver
     ...
     REAL,  OPTIONAL,DIMENSION( ims:ime, jms:jme ), INTENT(INOUT) :: SR_URB2D, SB_URB2D, SG_URB2D, RR_URB2D, RB_URB2D, RG_URB2D, HR_URB2D, HB_URB2D, HG_URB2D, ELER_URB2D, ELEB_URB2D, ELEG_URB2D, G0R_URB2D, G0B_URB2D, G0G_URB2D  !cye-add
...
! output variables urban --> lsm
     ...
     REAL :: CHS2_URB
     REAL :: UST_URB
     REAL :: SR_URB, SB_URB, SG_URB, RR_URB, RB_URB, RG_URB, HR_URB, HB_URB, HG_URB, ELER_URB, ELEB_URB, ELEG_URB, G0R_URB, G0B_URB, G0G_URB  !cye-add
...
! Local variables for multi-layer UCM
     ...
     REAL :: frc_urb,lb_urb
     REAL :: check
     REAL :: solar_time !cye-add 2025-06-16
...
! Call urban
    ...
    !  DECLIN_URB = -23.44 * COS(  (360/365) * (jday + 10) ) *  (3.1415926/180.0)  ! declination in radians
    ! DECLIN_URB = 23.44*COS((360./365.)*(julian+10.))*(3.1415926/180.0)  ! declination in radians cye
    DECLIN_URB = 23.44 * SIN(((2 * 3.1415926) / 365) * (julian - 81))*((3.1415926/180.0)) !
    solar_time = (julian - int(julian))*24 + xlong(i,j)*(180/3.1415926)/15.0
             if ( solar_time < 0.0 ) solar_time = solar_time + 24.0

    OMG_URB =  (solar_time-12)*15*0.0175 !cye


! if (I==209 .and. UTYPE_URB == 8) then
!           open( 30, file= 'output.csv',position="append" )!cye-write
!           write(30,*) julian,julyr,jmonth,jday,DECLIN_URB,solar_time,OMG_URB
!           close(30) 
! endif

    CALL urban(I,J,LSOLAR_URB,                                   & ! I
         ...
         GZ1OZ0_URB,                                                                   & !O
           ...
           TGRL_URB,    SMR_URB,   CMGR_URB,   CHGR_URB,   jmonth,jday,                     & ! H
          DRELR_URB,  DRELB_URB,                                                       & ! H
          DRELG_URB,FLXHUMR_URB,FLXHUMB_URB,FLXHUMG_URB, SR_URB, SB_URB, SG_URB, RR_URB, RB_URB, RG_URB, HR_URB, HB_URB, HG_URB, ELER_URB, ELEB_URB, ELEG_URB, G0R_URB, G0B_URB, G0G_URB )

! ! if ( abs( xlong(i,j)*(180/3.1415926) - 114.175 ) < 0.1 .and. abs(xlat(i,j)*(180/3.1415926)-22.276944) < 0.1) then
! if (I==209 .and. UTYPE_URB == 8) then
!           open( 30, file= 'output.csv',position="append" )!cye-write
!           ! write(30,*) 'LH_URB = ', LH_URB, 'LH = ', LH(I,J), 'FRC_URB2D(I,J) * LH_URB  + (1-FRC_URB2D(I,J)) * LH(I,J)   = ',FRC_URB2D(I,J) * LH_URB  + (1-FRC_URB2D(I,J)) * LH(I,J)  
!           write(30,*) 'SH_URB = ', SH_URB, 'HFX = ', HFX(I,J), 'FRC_URB2D(I,J) * SH_URB  + (1-FRC_URB2D(I,J)) * HFX(I,J)   = ',FRC_URB2D(I,J) * SH_URB  + (1-FRC_URB2D(I,J)) * HFX(I,J)  
!           ! write(30,*) 'T2MVXY(I,J) = ', T2MVXY(I,J), 'T2MBXY(I,J) = ', T2MBXY(I,J), '(FVEGXY(I,J)*T2MVXY(I,J) + (1.-FVEGXY(I,J))*T2MBXY(I,J)) = ',(FVEGXY(I,J)*T2MVXY(I,J) + (1.-FVEGXY(I,J))*T2MBXY(I,J)), & 
!           !   'TH2_URB2D(i,j) = ', TH2_URB2D(i,j), 'TH2_URB2D(i,j)/((1.E5/P8W3D(I,KTS,J))**RCP) = ', TH2_URB2D(i,j)/((1.E5/P8W3D(I,KTS,J))**RCP), &
!           !    '(FVEGXY(I,J)*T2MVXY(I,J) + (1.-FVEGXY(I,J))*T2MBXY(I,J))*(1.-FRC_URB2D(I,J)) + (TH2_URB2D(i,j)/((1.E5/P8W3D(I,KTS,J))**RCP))*FRC_URB2D(I,J) = ',(FVEGXY(I,J)*T2MVXY(I,J) &
!           !     + (1.-FVEGXY(I,J))*T2MBXY(I,J))*(1.-FRC_URB2D(I,J)) + (TH2_URB2D(i,j)/((1.E5/P8W3D(I,KTS,J))**RCP))*FRC_URB2D(I,J)
          
!         !   write(30,*) xlong(i,j)*(180/3.1415926),xlat(i,j)*(180/3.1415926),I,J,IVGTYP(I,J), UTYPE_URB, &
!         !   HFX(I,J),SH_URB,GRDFLX(I,J),G_URB,T2(I,J),TH2_URB2D(i,j)/((1.E5/P8W3D(I,KTS,J))**RCP),FRC_URB2D(I,J)
!         !   write(30,*) 'SH_URB = ', SH_URB,'HFX = ',HFX(I,J),'GRDFLX = ',GRDFLX(I,J), &
!         !   & 'G_URB = ',G_URB,'T2 = ',T2(I,J),'TH2_URB = ',TH2_URB2D(I,J)/((1.E5/P8W3D(I,KTS,J))**RCP), &
!         !   & 'FRC_URB = ',FRC_URB2D(I,J)
!         !   write(30,*) 'LSOLAR_URB = ',LSOLAR_URB,                                                             & ! I
!         !   'num_roof_layers = ',num_roof_layers, 'num_wall_layers = ',num_wall_layers, 'num_road_layers = ',num_road_layers,                           & ! C
!         !     'DZR = '   , DZR,        'DZB = ',DZB,        'DZG = ',DZG, & ! C
!         !   'UTYPE_URB = ',     UTYPE_URB,     'TA_URB = ',     TA_URB,     'QA_URB = ',     QA_URB,     'UA_URB = ',   UA_URB,   'U1_URB = ',  U1_URB,  'V1_URB = ', V1_URB, 'SSG_URB = ', SSG_URB,   & ! I
!         !    'SSGD_URB = ',SSGD_URB,   'SSGQ_URB = ',SSGQ_URB,    'LLG_URB = ',   LLG_URB,   'RAIN_URB = ', RAIN_URB, 'RHOO_URB = ', RHOO_URB,                     & ! I
!         !      'ZA_URB = ',ZA_URB,   'DECLIN_URB = ',   DECLIN_URB,   'COSZ_URB = ',    COSZ_URB,   'OMG_URB = ',   OMG_URB,                               & ! I
!         !    'XLAT_URB = ',XLAT_URB,   'DELT_URB = ',    DELT_URB,   'ZNT_URB = ',   ZNT_URB,                                           & ! I
!         !     'CHS_URB = ',   CHS_URB,   'CHS2_URB = ',   CHS2_URB,                                                       & ! I
!         !      'TR_URB = ',     TR_URB,     'TB_URB = ',     TB_URB,     'TG_URB = ',     TG_URB,     'TC_URB = ',   TC_URB,   'QC_URB = ',   QC_URB,   'UC_URB = ',   UC_URB,           & ! H
!         !     'TRL_URB = ',    TRL_URB,    'TBL_URB = ',    TBL_URB,    'TGL_URB = ',    TGL_URB,                                           & ! H
!         !    'XXXR_URB = ', XXXR_URB,   'XXXB_URB = ',   XXXB_URB,   'XXXG_URB = ',   XXXG_URB,   'XXXC_URB = ',   XXXC_URB,                               & ! H
!         !      'TS_URB = ', TS_URB,     'QS_URB = ',QS_URB,     'SH_URB = ',     SH_URB,     'LH_URB = ', LH_URB, 'LH_KINEMATIC_URB = ', LH_KINEMATIC_URB,             & ! O
!         !      'SW_URB = ',    SW_URB,    'ALB_URB = ',     ALB_URB,     'LW_URB = ',      LW_URB,   'G_URB = ',   G_URB,   'RN_URB = ', RN_URB, 'PSIM_URB = ', PSIM_URB, 'PSIH_URB = ', PSIH_URB, & ! O
!         !  'GZ1OZ0_URB = ',     GZ1OZ0_URB,                                                              & !O
!         !     'CMR_URB = ',    CMR_URB,    'CHR_URB = ',    CHR_URB,    'CMC_URB = ',    CMC_URB,    'CHC_URB = ',   CHC_URB,                               &
!         !     'U10_URB = ',    U10_URB,    'V10_URB = ',    V10_URB,    'TH2_URB = ',     TH2_URB,     'Q2_URB = ',   Q2_URB,                               & ! O
!         !     'UST_URB = ',     UST_URB,   'mh_urb = ',   mh_urb,     'stdh_urb = ',     stdh_urb,   'lf_urb = ',   lf_urb,   'lp_urb = ',   lp_urb,                     & ! 0
!         !     'hgt_urb = ',    hgt_urb,    'frc_urb = ',     frc_urb,     'lb_urb = ',      lb_urb,      'check = ',      check,    'CMCR_URB = ', CMCR_URB, 'TGR_URB = ', TGR_URB,             & ! H
!         !    'TGRL_URB = ',    TGRL_URB,   'SMR_URB = ',   SMR_URB,   'CMGR_URB = ',   CMGR_URB,   'CHGR_URB = ',   CHGR_URB,   'jmonth = ',   jmonth,                     & ! H
!         !   'DRELR_URB = ',  DRELR_URB,                                                       & ! H
!         !   'DRELG_URB = ',DRELG_URB, 'FLXHUMR_URB = ',FLXHUMR_URB, 'FLXHUMB_URB = ',FLXHUMB_URB, 'FLXHUMG_URB = ',FLXHUMG_URB
!           write(30,*) ' '
!           close(30) 
! endif
...
! if ( I ==78 .and. UTYPE_URB == 8) then
!           open( 30, file= 'output.csv',position="append" )!cye-write
!           write(30,*) I, J
!           write(30,*) ' UST(I,J)  = ',  UST(I,J) , 'UST_URB = ', UST_URB, 'FRC_URB2D(I,J) * UST_URB + (1-FRC_URB2D(I,J)) * UST(I,J)   = ',FRC_URB2D(I,J) * UST_URB + (1-FRC_URB2D(I,J)) * UST(I,J)
!           write(30,*) ' '
!           close(30) 
! endif
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!cye
    ...
    HB_URB2D(I,J) = HB_URB
    HG_URB2D(I,J) = HG_URB
    ELER_URB2D(I,J) = ELER_URB
    ELEB_URB2D(I,J) = ELEB_URB
    ELEG_URB2D(I,J) = ELEG_URB
    G0R_URB2D(I,J) = G0R_URB
    G0B_URB2D(I,J) = G0B_URB
    G0G_URB2D (I,J) = G0G_URB
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!cye
...
! if (I==209 ) then
! open( 30, file= 'output.csv',position="append" )!cye-write
!                     write(30,*) 'FVEGXY(I,J) = ',FVEGXY(I,J),'Q2MVXY(I,J) = ',Q2MVXY(I,J), &
!                     ' (1.-FVEGXY(I,J)) = ',(1.-FVEGXY(I,J)),'Q2MBXY(I,J) = ',Q2MBXY(I,J), &
!                     'FRC_URB2D(I,J) = ',FRC_URB2D(I,J),' Q2_URB2D(I,J) = ',Q2_URB2D(I,J), &
!                     'QA_URB = ', QA_URB
!                     write(30,*) ' '
! close(30) 
! endif
