...
! -------------------------------------------------
    ...
    if ( NoahmpIO%sf_urban_physics > 0 )  then
       ...
       if ( .not. allocated (NoahmpIO%uc_urb2d)     ) allocate ( NoahmpIO%uc_urb2d      (its:ite)         )
        ...
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!cye
        if ( .not. allocated (NoahmpIO%SR_URB2D   )     ) allocate ( NoahmpIO%SR_URB2D       (its:ite)         )
        if ( .not. allocated (NoahmpIO%SB_URB2D   )     ) allocate ( NoahmpIO%SB_URB2D       (its:ite)         )
        if ( .not. allocated (NoahmpIO%SG_URB2D   )     ) allocate ( NoahmpIO%SG_URB2D       (its:ite)         )
        if ( .not. allocated (NoahmpIO%RR_URB2D   )     ) allocate ( NoahmpIO%RR_URB2D       (its:ite)         )
        if ( .not. allocated (NoahmpIO%RB_URB2D   )     ) allocate ( NoahmpIO%RB_URB2D       (its:ite)         )
        if ( .not. allocated (NoahmpIO%RG_URB2D   )     ) allocate ( NoahmpIO%RG_URB2D       (its:ite)         )
        if ( .not. allocated (NoahmpIO%HR_URB2D   )     ) allocate ( NoahmpIO%HR_URB2D       (its:ite)         )
        if ( .not. allocated (NoahmpIO%HB_URB2D   )     ) allocate ( NoahmpIO%HB_URB2D       (its:ite)         )
        if ( .not. allocated (NoahmpIO%HG_URB2D   )     ) allocate ( NoahmpIO%HG_URB2D       (its:ite)         )
        if ( .not. allocated (NoahmpIO%ELER_URB2D )     ) allocate ( NoahmpIO%ELER_URB2D     (its:ite)         )
        if ( .not. allocated (NoahmpIO%ELEB_URB2D )     ) allocate ( NoahmpIO%ELEB_URB2D     (its:ite)         )
        if ( .not. allocated (NoahmpIO%ELEG_URB2D )     ) allocate ( NoahmpIO%ELEG_URB2D     (its:ite)         )
        if ( .not. allocated (NoahmpIO%G0R_URB2D  )     ) allocate ( NoahmpIO%G0R_URB2D      (its:ite)         )
        if ( .not. allocated (NoahmpIO%G0B_URB2D  )     ) allocate ( NoahmpIO%G0B_URB2D      (its:ite)         )
        if ( .not. allocated (NoahmpIO%G0G_URB2D  )     ) allocate ( NoahmpIO%G0G_URB2D      (its:ite)         )
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!cye
    ...
    if ( NoahmpIO%sf_urban_physics > 0 ) then
       ...
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!cye
      NoahmpIO%SR_URB2D   = undefined_real_neg
      NoahmpIO%SB_URB2D   = undefined_real_neg
      NoahmpIO%SG_URB2D   = undefined_real_neg
      NoahmpIO%RR_URB2D   = undefined_real_neg
      NoahmpIO%RB_URB2D   = undefined_real_neg
      NoahmpIO%RG_URB2D   = undefined_real_neg
      NoahmpIO%HR_URB2D   = undefined_real_neg
      NoahmpIO%HB_URB2D   = undefined_real_neg
      NoahmpIO%HG_URB2D   = undefined_real_neg
      NoahmpIO%ELER_URB2D = undefined_real_neg
      NoahmpIO%ELEB_URB2D = undefined_real_neg
      NoahmpIO%ELEG_URB2D = undefined_real_neg
      NoahmpIO%G0R_URB2D  = undefined_real_neg
      NoahmpIO%G0B_URB2D  = undefined_real_neg
      NoahmpIO%G0G_URB2D  = undefined_real_neg
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!cye
